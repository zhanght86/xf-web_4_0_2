    ...
    '$scope', 'localStorageService' ,"$state" ,"ngDialog",function ($scope,localStorageService, $state,ngDialog) {
        ...
        function singleEdit(item){
            ...
            httpRequestPost("/api/modeling/concept/correction/update",{
                ...
                console.log(item);
                console.log(item.correctionConceptId,$scope.vm.applicationId,$scope.vm.key,typeof $scope.vm.modifier,$scope.vm.term);
                console.log(data);
                layer.msg("编辑成功");
                $state.reload()
            },function(){
...
angular.module('materialManagement').controller('faqChatController', [
    '$scope',"$state","ngDialog", function ($scope,$state,ngDialog) {
       ...
       //刪除
        ...
        function scan(){
            if(check()){
                //$state.go("materialManagement.chatKnowledgeBasePreview",{"scanData",})
            }
        }
        //保存
        function save(){
        ...
        function check(){
            if($scope.vm.standardQuestion){
                layer.msg("标准问不能为空");
                return false
            }else if($scope.vm.extendedQuestionArr){
                layer.msg("扩展问不能为空");
                return false;
            }else if($scope.vm.contentArr){
                layer.msg("知识内容不能为空");
                return false
            }else{
                return true
